...
This document provides comprehensive context for AI agents working with this repository.
...
**Language**: Swift  
**Platform**: macOS 13.0+  
**Purpose**: Display GitHub Copilot premium request usage as a percentage in the macOS menu bar
...
├── GithubCopilotNotify/
│   ├── AppDelegate.swift           # Menu bar UI, timer, and main app logic
│   ├── CopilotSessionAPI.swift     # GitHub session-based API client
│   ├── GitHubWebAuthClient.swift   # WebKit-based browser login flow
│   ├── CopilotAPI.swift            # Legacy OAuth-based API (not currently used)
│   ├── GitHubOAuthClient.swift     # Legacy OAuth flow (not currently used)
...
- **Models**:
  - `CopilotEntitlement`: API response containing license type, quotas, plan info
...
- **Key Fields**:
  ...
  - `premiumInteractionsPercentage`: Percentage of quota **remaining** (e.g., 92.9% means 92.9% left)
  - `resetDate`: When the quota resets (monthly)
  - `overagesEnabled`: Whether overages are enabled
- **Calculation**: `usedPercentage = 100.0 - premiumInteractionsPercentage` (API returns remaining, we display used)
- **Error Handling**: Returns 401/403 if cookies expired, user needs to re-sign in
...
- **Security**: Uses nonPersistent WebView session, saves to system cookie store
...
- **States**:
  - "Copilot: XX.X%" - Normal operation showing premium request usage percentage
...
xcodebuild -project GithubCopilotNotify.xcodeproj -scheme GithubCopilotNotify -configuration Debug build

# Release build via command line
xcodebuild -project GithubCopilotNotify.xcodeproj -scheme GithubCopilotNotify -configuration Release build
...
open ~/Library/Developer/Xcode/DerivedData/GithubCopilotNotify-*/Build/Products/Debug/GithubCopilotNotify.app
...
6. Check Console.app for debug logs if issues occur (look for emoji markers like 🔍, ✅, ❌, 🍪)

## API Integration Details

### GitHub Internal API

**Entitlement Endpoint** (`https://github.com/github-copilot/chat/entitlement`):
...
**Key Fields**:
- `premiumInteractionsPercentage`: Percentage of premium requests **remaining** (API value)
...
**Note**: The API returns percentage **remaining** (free), but the app displays percentage **used**. The calculation is: `usedPercentage = 100.0 - premiumInteractionsPercentage`. So if the API returns 92.9% remaining, the menu bar shows "7.1%" used.
...
- Alternative: Show percentage **remaining** (`premiumInteractionsPercentage` directly)
...
```swift
if let cookies = HTTPCookieStorage.shared.cookies(for: URL(string: "https://github.com")!) {
...
1. **Individual User Only**: Shows personal premium request usage, not organization metrics
...
7. **Internal API**: Uses GitHub's internal API which may change without notice
...
- Clean build folder: `xcodebuild clean` or Product -> Clean Build Folder in Xcode
- Check for missing files in Xcode project navigator
- Verify all Swift files are included in the target's Compile Sources build phase
